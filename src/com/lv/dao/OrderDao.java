...
public class OrderDao {
    ...
    public void addProduct(Order order) throws SQLException {
        ...
        String sql = "insert into orders(id,user_id,money,receiveAddress,receiveName,receivePhone,paystate,ordertime)values(?,?,?,?,?,?,null,?)";
        // 2.生成执行sql语句的QueryRunner,不传递参数
        QueryRunner runner = new QueryRunner();
        // 3.执行update()方法插入数据
        runner.update(DataSourceUtils.getConnection(), sql, order.getId(),
                order.getUser().getId(),order.getMoney(),order.getReceiverAddress(),order.getReceiverName(),order.getReceiverPhone(),date);
    }
    /**
     * 查找用户所属订单
     */
